  ...
  <section>
    ...
    <div class="content basepage fullheight searchPage" id="content" >
      ...
      <div v-if="searchResult" class="search-result-holder">
        ...
        <button v-else v-for="(search, index) in searchResult" :key="index" class="search-result" v-on:click="changeLocation(search.Key, search.LocalizedName )" > {{ search.LocalizedName }} / {{ search.Country.LocalizedName }}</button>
      </div>
    </div>
    <Footer />
  </section>
